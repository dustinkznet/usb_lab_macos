...
class SpeedTestMenu:
    ...
    def __init__(self, inspector: DiskInspector, db: DatabaseManager, settings):
        ...
        self.test_engine = DriveTestEngine(db)

    def show(self):
        """Display speed test menu and handle user interaction."""
        clear_screen()
        print_header()
    ...
    def _run_comprehensive_tests(self, disk: PhysicalDisk, adapter_info: str = None):
        ...
        results = self.test_engine.run_comprehensive_test_suite(testable_partition, drive_id, adapter_info=adapter_info)
